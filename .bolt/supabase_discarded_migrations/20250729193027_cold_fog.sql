/*
  ...
  1. Nova Tabela
    - `reservas`
      ...
      - `id_mesa` (integer, not null, check: between 1 and 98)
      - `data_reserva` (date, not null)
      - `horario_reserva` (time, not null)
      - `observacoes` (text, default: '')
      - `status` (text, default: 'ativa', check: in 'ativa', 'cancelada', 'finalizada')
  ...
  3. Segurança
    ...
    - Política para usuários autenticados gerenciarem reservas
*/
...
-- RLS desabilitado conforme schema atual, mas com política para autenticados
-- ALTER TABLE reservas ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Authenticated users can manage reservas"
  ON reservas
  FOR ALL
  TO authenticated
  USING (true)
