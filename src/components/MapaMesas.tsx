...
const MapaMesas: React.FC<MapaMesasProps> = ({ 
    ...
    return mesasLayout.map(mesa => {
      ...
      if (reservaEmEdicao) {
        if (idsMesasSelecionadas.has(mesa.id)) {
          ...
          status = 'disponivel'; 
        } else if (reserva && (reserva.status === 'confirmada' || reserva.status === 'pendente')) {
        status = 'ocupada';
        }
      } else {
        if (reserva && (reserva.status === 'confirmada' || reserva.status === 'pendente')) {
          status = 'ocupada'
        }
        if (idsMesasSelecionadas.has(mesa.id)) {
          status = 'selecionada'
  ...
  const podeSelecionar = (mesa: Mesa) => {
    ...
    if (mesa.reserva && (mesa.reserva.status === 'confirmada' || mesa.reserva.status === 'pendente')) return true;
